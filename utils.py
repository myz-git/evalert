...
def predict_icon_status(image, clf, scaler):
    """通过机器学习的模型来验证图标状态"""
    # 使用BGR颜色空间计算直方图，与训练时的特征提取方法保持一致
    # 训练时使用: cv2.calcHist([image], [0, 1, 2], None, [16, 16, 16], [0, 256, 0, 256, 0, 256])
...
def find_icon(template, width, height, clf, scaler, max_attempts=3, offset_x=0, offset_y=0, region=None, exflg=False):
    if region is None:
        fx, fy = pyautogui.size()
        region = (0, 0, fx, fy)
